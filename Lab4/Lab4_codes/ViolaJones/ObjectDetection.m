...
%Code that leaves out overlapping detections
Objects1 = [];
k = 1;
%Loop through detections and check if the overlapping area is not null
for i = 1: size(Objects,1)
    for j = i + 1: size(Objects,1)
        overlap = rectint(Objects(i,:,:,:), Objects(j,:,:,:));
        if(overlap > 0)
            Objects(j,:,:,:) = [0 0 0 0];
        end
    end 
        if (~isequal(Objects(i,:,:,:),[0 0 0 0]))
        Objects1(k,:,:,:) = Objects(i,:,:,:);
        k = k + 1;
        end
end

%Objects1 contains only unique non-overlapping detections
Objects = Objects1;
